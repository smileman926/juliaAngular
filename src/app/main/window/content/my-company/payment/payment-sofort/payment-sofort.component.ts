...
@Component({
  selector: 'app-payment-sofort',
  templateUrl: './payment-sofort.component.pug',
  styleUrls: ['./payment-sofort.component.sass']
})
export class PaymentSofortComponent implements OnInit {
  ...
  public async init(): Promise<void> {
    const [val1, val3, val4, val5] = await Promise.all([
      ...
      this.apiPayment.getPaymentMethodLocaleModel({p3: 4}).toPromise(),
      this.apiPayment.getCountryInfoList().toPromise(),
      this.apiPayment.getPaymentMethodCountryModel({p3: 4}).toPromise()
    ]);
    ...
    this.form = new FormGroup({
      pm_accountId: new FormControl(this.payTypeList.filter(item => item.pm_name === 'Sofort')[0].pm_accountId),
      pm_projectId: new FormControl(this.payTypeList.filter(item => item.pm_name === 'Sofort')[0].pm_projectId),
      pm_notificationPassword: new FormControl(
        this.payTypeList.filter(item => item.pm_name === 'Sofort')[0].pm_notificationPassword
      ),
      pm_projectPassword: new FormControl(
        this.payTypeList.filter(item => item.pm_name === 'Sofort')[0].pm_projectPassword
      ),
    });
  }
  ...
  public async save(): Promise<void> {
    ...
    const body = {
      ...
      pm_projectId: formVals.pm_projectId,
      pm_notificationPassword: formVals.pm_notificationPassword,
      pm_projectPassword: formVals.pm_projectPassword,
      pm_id: this.payTypeList.filter(item => item.pm_name === 'Sofort')[0].pm_id
    };

    await this.apiPayment.putBodyRequest(
      'apiPayment/paymentMethod',
      this.payTypeList.filter(item => item.pm_name === 'Sofort')[0].pm_id,
      body
    ).toPromise();
  }
  ...
  public async changeTranslate(isFirst: boolean, isChargeText?: boolean) {
    ...
    modalBody.init('4', isFirst, isChargeText);
    modal.save.subscribe( async () => {
      const result: any = await modalBody.save();
      modal.close(!!result.res);
      const initialValue = await this.getDefaultTranslateText('4', isFirst, isChargeText);
      if (result.type) {
